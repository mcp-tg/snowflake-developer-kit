"""
DDL Manager for Snowflake MCP Server

This module provides a DDLManager class that encapsulates DDL operations,
following the simple connection pattern from the reference implementation.

The DDLManager creates fresh connections for each instance, eliminating
the need for complex connection pooling or persistence.
"""

from typing import Dict, List, Optional, Union
from ..core.snowflake_utils import get_snowflake_connection
from ..core.exceptions import DDLException
...
class DDLManager:
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize the DDL manager with a fresh Snowflake connection.
        ...
        try:
            ...
            raise DDLException(f"Failed to establish connection: {str(e)}", "CONNECTION", "")
    ...
    def execute_ddl(self, ddl_statement: str) -> Dict[str, Union[bool, str, List[str]]]:
        ...
        try:
            ...
            raise DDLException(f"Error executing DDL: {str(e)}", "EXECUTE", ddl_statement)
    ...
    def alter_table(
        ...
        if alter_type == "ADD":
            if not (column_name and data_type):
                raise DDLException("Column name and data type required for ADD operation", "ALTER", table_name)
            ...
            if not column_name:
                raise DDLException("Column name required for DROP operation", "ALTER", table_name)
            ddl = f"ALTER TABLE {table_name} DROP COLUMN {column_name}"
            
        elif alter_type == "RENAME":
            if not (column_name and new_name):
                raise DDLException("Column name and new name required for RENAME operation", "ALTER", table_name)
            ddl = f"ALTER TABLE {table_name} RENAME COLUMN {column_name} TO {new_name}"
            
        elif alter_type == "ALTER":
            if not (column_name and data_type):
                raise DDLException("Column name and data type required for ALTER operation", "ALTER", table_name)
            ddl = f"ALTER TABLE {table_name} ALTER COLUMN {column_name} SET DATA TYPE {data_type}"
            
        else:
            raise DDLException(f"Unsupported alter type: {alter_type}", "ALTER", table_name)
    ...
    def alter_schema(
        ...
        if new_name:
            ...
            current_schema = schema_name.split('.')[-1]
            ddl = f"ALTER SCHEMA {schema_name} RENAME TO {new_database}.{current_schema}"
        else:
            raise DDLException("Either new_name or new_database must be provided", "ALTER", schema_name)
